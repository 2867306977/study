function myAjax(options) {
  ...
  let { url, data, type = 'get', dataType = 'json', timeout, success, error, complete, beforeSend } = options
  ...
  //beforeSend()方法是在请求之前进行一些处理在判断需不需要发送请求(例如:发送之前做表单验证),beforeSend有四种情况
  /* 
  1. 如果不写,则继续发送请求 //返回undefined
  2. 如果写了,但没有return,也是继续发送请求(如果return了其他,视为没有return)  //返回undefined
  3. 如果写了并且return true,则发送请求  //true
  4. 如果写了并且return false,则不发送请求  //false
   */
  //接收它们的返回值进行判断
  const isSend = beforeSend && beforeSend()
  //因为只有return false不发送请求那么就判断return的是否是false,是则直接return
  if (isSend === false) return //这里必须全等判断 如果return 0 也是视为没有return
    ...
    */
    if (xhr.readyState === 4) {
      ...
      if (xhr.status === 200) {
        ...
        //判断是否写了success属性,如果没有写则不需要执行success方法
        success && success(value) //&& 如果第一个返回的是false 则不执行第二条语句
      } else {
        //失败
        error && error(xhr)
      }
